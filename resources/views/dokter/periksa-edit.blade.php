  ...
  <div class="container-fluid">
        ...
        <div class="card h-100 shadow-sm">
            ...
            <div class="card-body">
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-7 mb-1">{{ $periksa->pasien->nama }}</dd>
                    <dt class="col-sm-5">Email</dt>
                    <dd class="col-sm-7 mb-1">{{ $periksa->pasien->email }}</dd>
                    <dt class="col-sm-5">No. HP</dt>
                    <dd class="col-sm-7 mb-1">{{ $periksa->pasien->no_hp }}</dd>
                    <dt class="col-sm-5">Alamat</dt>
                    <dd class="col-sm-7 mb-1">{{ $periksa->pasien->alamat }}</dd>
                    <dt class="col-sm-5">Tanggal Periksa</dt>
                    <dd class="col-sm-7 mb-1">{{ \Carbon\Carbon::parse($periksa->tgl_periksa)->format('d M Y H:i') }}</dd>
                    <dt class="col-sm-5">Keluhan</dt>
                    <dd class="col-sm-7 mb-1">{{ $periksa->keluhan ?? '-' }}</dd>
                </dl>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-12">
        <div class="card shadow-sm">
            ...
            <form action="{{ route('dokter.periksa.update', $periksa->id) }}" method="POST" class="m-0">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan" rows="4" required>{{ old('catatan', $periksa->catatan_dokter) }}</textarea>
                                @error('catatan')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('biaya_periksa') is-invalid @enderror" id="biaya_periksa" name="biaya_periksa" value="{{ old('biaya_periksa', $periksa->biaya_periksa ?? 150000) }}" required>
                                @error('biaya_periksa')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="select2" multiple="multiple" name="obat_ids[]" data-placeholder="Pilih obat" style="width: 100%;">
                                    @foreach($obats as $obat)
                                        <option value="{{ $obat->id }}" {{ (collect(old('obat_ids', $periksa->obat->pluck('id')->toArray()))->contains($obat->id)) ? 'selected' : '' }}>
